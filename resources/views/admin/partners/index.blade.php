...
<div class="container mx-auto p-4">
    <div class="bg-white shadow-lg rounded-lg p-6">
        ...
        <table id="partnersTable" class="w-full mt-4 bg-white shadow-md rounded-lg text-gray-700 border border-gray-200">
            <thead class="text-gray-600 text-sm uppercase border-b border-gray-300">
                <tr>
                    <th class="p-1 text-left">Nom</th>
                    <th class="p-1 text-left">Logo</th>
                    <th class="p-1 text-left">URL</th>
            ...
            <tbody class="text-gray-600 text-sm">
                @foreach ($partners as $partner)
                    <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration-200">
                        <td class="p-1">{{ $partner->name }}</td>
                        <td class="p-1">
                            @if($partner->logo)
                                <img src="{{ asset('storage/' . $partner->logo) }}" alt="Logo" class="w-10 h-10 rounded">
                            @else
                                N/A
                            @endif
                        </td>
                        ...
                        <td class="p-1 flex gap-3">
                            <a href="{{ route('admin.partners.show', $partner->id) }}" class="text-gray-500 hover:text-gray-900">
                                <i class="fa fa-eye"></i>
                            </a>
                            <a href="{{ route('admin.partners.edit', $partner->id) }}" class="text-blue-500 hover:text-blue-700">
