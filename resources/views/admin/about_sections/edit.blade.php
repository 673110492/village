...
<div class="p-6 md:p-10">
    <div class="bg-white rounded-2xl shadow-md p-6 max-w-4xl mx-auto">
        ...
        <form action="{{ route('admin.about_sections.update', $about->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                @if($about->image)
                  <img src="{{ asset('storage/' . $about->image) }}" alt="Image actuelle" class="mt-2 w-24 h-24 object-cover rounded">
                @endif
            ...
            <div>
                ...
                <textarea name="contenu" id="contenu" class="richtext w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-yellow-300">{{ old('contenu', $about->contenu) }}</textarea>
            </div>
