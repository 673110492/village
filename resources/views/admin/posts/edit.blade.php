...
@section('pageSubTitle', 'Actualités / Édition')
...
<div class="container mx-auto p-4">
    <div class="bg-white shadow-lg rounded-lg p-6 max-w-3xl mx-auto">
        ...
        <h2 class="text-2xl font-semibold text-gray-700 mb-4">
            Modifier l’actualité : <span class="text-blue-600">{{ $post->slug }}</span>
        </h2>
        ...
        <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <!-- Titre -->
            <div>
                <label for="slug" class="block text-sm font-medium text-gray-700 mb-1">Titre de l'actualité</label>
            ...
            <div>
                ...
                @if($post->cover_image)
                  <img src="{{ asset('storage/' . $post->cover_image) }}" alt="Image actuelle" class="mt-2 w-24 h-24 object-cover rounded">
                @endif
            ...
            <div>
                ...
                <textarea name="contenu" id="contenu" class="richtext w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-yellow-300">{{ old('contenu', $post->contenu) }}</textarea>
            </div>
