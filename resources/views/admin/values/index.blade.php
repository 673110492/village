...
<div class="container mx-auto p-4">
    <div class="bg-white shadow-lg rounded-lg p-6">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('admin.values.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded inline-flex items-center">
                <i class="fa fa-plus"></i>
            </a>
        </div>
        <hr class="border-t-3 border-blue-500 mb-4">
        <table id="valuesTable" class="w-full mt-4 bg-white shadow-md rounded-lg text-gray-700 border border-gray-200">
            <thead class="text-gray-600 text-sm uppercase border-b border-gray-300">
                <tr>
                    <th class="p-1 text-left">Titre</th>
            ...
            <tbody class="text-gray-600 text-sm">
                @foreach ($values as $value)
                    <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration-200">
                        ...
                        <a href="{{ route('admin.values.show', $value->id) }}" class="text-gray-500 hover:text-gray-900">
                        <i class="fa fa-eye"></i> <!-- Icône pour voir les détails -->
                        </a>
                            <a href="{{ route('admin.values.edit', $value->id) }}" class="text-blue-500 hover:text-blue-700">
                                <i class="fa fa-pencil"></i>
                            </a>

                            <form action="{{ route('admin.values.destroy', $value->id) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer cette valeur ?');">
                                @csrf
                                @method('DELETE')
                            ...
                            <form action="{{ route('admin.values.toggleStatus', $value->id) }}" method="POST">
                                @csrf
                                @method('PATCH')
