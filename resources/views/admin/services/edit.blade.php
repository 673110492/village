...
@section('pageSubTitle', 'Services / Édition')
...
<div class="container mx-auto p-4">
    <div class="bg-white shadow-lg rounded-lg p-6 max-w-3xl mx-auto">
        ...
        <h2 class="text-2xl font-semibold text-gray-700 mb-4">
            Modifier le service : <span class="text-blue-600">{{ $service->slug }}</span>
        </h2>
        ...
        <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                @if($service->image)
                  <img src="{{ asset('storage/' . $service->image) }}" alt="Image actuelle" class="mt-2 w-24 h-24 object-cover rounded">
                @endif
            ...
            <div>
                ...
                <textarea name="description" id="description" class="richtext w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-yellow-300">{{ old('description', $service->description) }}</textarea>
            </div>
