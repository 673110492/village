...
@section('pageTitle', 'Modifier une mission')
@section('pageSubTitle', 'Missions / Édition')
...
<div class="container mx-auto p-4">
    <div class="bg-white shadow-lg rounded-lg p-6 max-w-3xl mx-auto">
        ...
        <h2 class="text-2xl font-semibold text-gray-700 mb-4">
            Modifier la mission : <span class="text-blue-600">{{ $mission->title }}</span>
        </h2>
        <hr class="border-t-3 border-blue-500 mb-6">

        <!-- Formulaire d'édition -->
        <form action="{{ route('admin.missions.update', $mission->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="8"
                          class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-yellow-300">{{ old('description', $mission->description) }}</textarea>
